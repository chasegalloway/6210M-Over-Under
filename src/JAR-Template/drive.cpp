...
void Drive::drive_with_velocity(float leftVoltage, float rightVoltage){
  float conversion_factor = 100.0 / 12.0; // Conversion factor from voltage to velocity
  float leftVelocity = leftVoltage * conversion_factor;
  float rightVelocity = rightVoltage * conversion_factor;
...
void Drive::set_drive_constants_velocity(float drive_max_velocity, float drive_kp, float drive_ki, float drive_kd, float drive_starti){
  this->drive_max_velocity = drive_max_velocity;
  this->drive_kp = drive_kp;
  this->drive_ki = drive_ki;
  this->drive_kd = drive_kd;
  this->drive_starti = drive_starti;
...
void Drive::drive_distance_velocity(float distance){
  drive_distance_velocity(distance, desired_heading, drive_max_velocity, heading_max_velocity, drive_settle_error, drive_settle_time, drive_timeout, drive_kp, drive_ki, drive_kd, drive_starti, heading_kp, heading_ki, heading_kd, heading_starti);
}

void Drive::drive_distance_velocity(float distance, float heading){
  drive_distance_velocity(distance, heading, drive_max_velocity, heading_max_velocity, drive_settle_error, drive_settle_time, drive_timeout, drive_kp, drive_ki, drive_kd, drive_starti, heading_kp, heading_ki, heading_kd, heading_starti);
}

void Drive::drive_distance_velocity(float distance, float heading, float drive_max_velocity, float heading_max_velocity){
  drive_distance_velocity(distance, heading, drive_max_velocity, heading_max_velocity, drive_settle_error, drive_settle_time, drive_timeout, drive_kp, drive_ki, drive_kd, drive_starti, heading_kp, heading_ki, heading_kd, heading_starti);
}

void Drive::drive_distance_velocity(float distance, float heading, float drive_max_velocity, float heading_max_velocity, float drive_settle_error, float drive_settle_time, float drive_timeout){
  drive_distance_velocity(distance, heading, drive_max_velocity, heading_max_velocity, drive_settle_error, drive_settle_time, drive_timeout, drive_kp, drive_ki, drive_kd, drive_starti, heading_kp, heading_ki, heading_kd, heading_starti);
}

void Drive::drive_distance_velocity(float distance, float heading, float drive_max_velocity, float heading_max_velocity, float drive_settle_error, float drive_settle_time, float drive_timeout, float drive_kp, float drive_ki, float drive_kd, float drive_starti, float heading_kp, float heading_ki, float heading_kd, float heading_starti){
  desired_heading = heading;
  PID drivePID(distance, drive_kp, drive_ki, drive_kd, drive_starti, drive_settle_error, drive_settle_time, drive_timeout);
  PID headingPID(reduce_negative_180_to_180(heading - get_absolute_heading()), heading_kp, heading_ki, heading_kd, heading_starti);
  float start_average_position = (get_left_position_in()+get_right_position_in())/2.0;
  ...
  while(drivePID.is_settled() == false){
    ...
    float clamped_drive_output = clamp(drive_output, -drive_max_velocity, drive_max_velocity);
    float clamped_heading_output = clamp(heading_output, -heading_max_velocity, heading_max_velocity);

    drive_with_velocity(clamped_drive_output + clamped_heading_output, clamped_drive_output - clamped_heading_output);
    task::sleep(10);
  }
  DriveL.stop(hold);
  DriveR.stop(hold);
